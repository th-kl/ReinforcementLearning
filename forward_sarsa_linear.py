...
def linear_forward_sarsa(params):
    ...
    agent = LinearSarsaAgent_V2(config=config, 
                                lamda=params['lamda'], 
                                n_tiles=params['n_tiles'], 
                                n_tilings=params['n_tilings'],
                                eta=params['eta'])
    
    env = agent.environment.env # Initialize environment
    ...
    for episode in range(agent.episodes):
        ...
        while not (terminated or truncated):
            ...
            if ready:
                ...
                s_t_a_t = agent.create_input_tensor(s_t_update, a_t_update)
                grads, q_pred = _compute_gradients(agent.model, s_t_a_t)
                pred_error = tf.squeeze(U - q_pred.numpy())
    ...
    results_dir = parent_dir+"/forward_linear_results"
    ...
    results_filepath = os.path.join(results_dir, f"{datetime.datetime.now().strftime('%m%d_%H%M%S')}_{params['job_id']}_forwardlinear_{params['trace']}_act_{params['activation']}_alpha_{str(params['alpha']).replace('.', '_')}_max_ep_{params['max_episode_steps']}_scaling_{params['observation_scaling']}_lamda_{str(params['lamda']).replace('.', '_')}_ntiles_{params['n_tiles']}_ntilings_{params['n_tilings']}.json")
    try:
